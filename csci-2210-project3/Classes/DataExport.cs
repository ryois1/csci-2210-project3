    ...
    public class DataExport
    {
        ...
        public static void ExportData(List<string> data, int simulationNumber)
        {
            ...
            if (!File.Exists(outputFile))
            {
                ...
                {
                    sw.WriteLine("Time Increment, Driver's name, Company Name, CrateID, Crate Value, Status");
                    sw.Close();
                }
            }
        ...
        public static void ExportResults(List<string> data, int simulationNumber)
        {
            ...
            if (!File.Exists(outputFile))
            {
                using (StreamWriter sw = File.CreateText(outputFile))
                {
                    sw.WriteLine("Open Docks, Longest Line, Total Trucks Processed, Total Crates Unloaded, Total Value, Average Crate Value, Average Truck Value, Total Time Dock was Open,"+
